...
export function OnlineQualifi({ className, data }: { className?: string; data?: OnlineProgramPageData }) {
  ...
  const crumb = hero?.breadcrumb ?? "Qualifi"
  const tagline = hero?.tagline ?? "QUALIFI ANH QUỐC"
  const heroTitle = hero?.title ?? "Học Chứng chỉ QUALIFI Anh Quốc trực tuyến cùng KVC Global"
  const heroSubtitle = hero?.subtitle ?? "Nâng tầm sự nghiệp với bằng cấp quốc tế được công nhận toàn cầu."
  const heroDescription = hero?.description ?? "KVC Global mang đến các chương trình đào tạo QUALIFI theo hình thức 100% Online, giúp học viên học tập linh hoạt, tiết kiệm chi phí và mở rộng cơ hội chuyển tiếp đại học, thạc sĩ tại Anh Quốc và nhiều quốc gia khác."
  ...
  const introTitle = intro?.title ?? "QUALIFI là gì?"
  const introImage = intro?.image ?? "https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?w=800&q=80&auto=format&fit=crop"
  const introImageAlt = intro?.imageAlt ?? "Qualifi program"
  ...
  const progressionTitle = progression?.title ?? "Cơ hội học tiếp"
  const progressionBody = progression?.body ?? "Sau khi hoàn thành các chương trình QUALIFI từ Level 4 trở lên, học viên có thể đủ điều kiện chuyển tiếp lên các chương trình đại học hoặc sau đại học tại nhiều trường đối tác theo quy định của từng trường."
