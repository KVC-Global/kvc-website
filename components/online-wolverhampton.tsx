        ...
        <Container>
          <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }}>
            <div className="mx-auto max-w-3xl lg:grid lg:max-w-none lg:grid-cols-[minmax(260px,0.7fr)_minmax(0,1.8fr)] lg:items-start lg:gap-14 xl:gap-20">
              <motion.div variants={fadeUpVariants} className="mb-12 text-center lg:mb-0 lg:pt-2 lg:text-left">
                <h2 className="text-balance font-heading text-2xl font-extrabold text-brand-blue sm:text-3xl lg:text-4xl lg:leading-tight">{supportTitle}</h2>
                <div className="mx-auto mt-2.5 h-0.5 w-12 rounded-full bg-brand-gold lg:mx-0 lg:mt-4" />
                <p className="mt-5 font-body text-sm leading-relaxed text-brand-dark/75">Đồng hành từ khi đăng ký đến khi tốt nghiệp</p>
              </motion.div>
              <ul className="space-y-4 lg:grid lg:grid-cols-2 lg:gap-4 lg:space-y-0">
                {supportItems.map((item, i) => (
                  <motion.li
                    ...
                    className="flex items-start gap-4 rounded-lg border border-border bg-white p-5 shadow-sm transition-all duration-300 lg:min-h-32 lg:p-6 lg:hover:-translate-y-0.5 lg:hover:border-brand-gold/30 lg:hover:shadow-md"
                  >
                    <div className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-brand-gold/10 lg:h-9 lg:w-9">
                      <Check className="h-4 w-4 text-brand-gold lg:h-[18px] lg:w-[18px]" strokeWidth={3} />
                    </div>
                    <span className="font-body text-sm leading-relaxed text-brand-dark/85 sm:text-base">{item}</span>
                  </motion.li>
                ))}
              </ul>
            </div>
