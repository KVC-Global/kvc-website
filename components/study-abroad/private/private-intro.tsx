...
import type { PrivateStudyIntroContent } from "@/sanity/private-study-page"

const DEFAULT_IMAGE = "https://images.unsplash.com/photo-1577896851231-70ef18881754?q=80&w=800&auto=format&fit=crop"

export function PrivateStudyAbroadIntro({ content }: { content?: PrivateStudyIntroContent }) {
  const imageUrl = content?.image
    ? urlFor(content.image).url()
    : DEFAULT_IMAGE

  const imageAlt = content?.imageAlt || "Học sinh trong lớp học trường quốc tế tại Singapore"
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col lg:col-span-7">
        ...
        >
          {content?.title || "Giới thiệu du học tư thục Singapore"}
        </h2>

        <div className="mt-6 font-body text-sm leading-relaxed text-brand-dark/90 sm:text-base">
          <p className="mb-4">
            {content?.paragraph1 || "Singapore không chỉ là trung tâm tài chính hàng đầu châu Á mà còn là một trong những điểm đến giáo dục quốc tế uy tín nhất thế giới."}
          </p>
          <p className="mb-4">
            {content?.paragraph2 ? (
              content.paragraph2
          ...
          <p className="mb-4">
            {content?.paragraph3 || "KVC Global hỗ trợ tư vấn và kết nối phụ huynh, học sinh với các trường quốc tế phù hợp nhất tại Singapore, đồng hành xuyên suốt từ khâu chọn trường đến khi học sinh hoàn toàn ổn định học tập."}
          </p>
        </div>
      </div>
    </section>
