  ...
  return (
    ...
    >
      <div className="text-center mb-12">
        ...
        >
          {content?.title || "Lộ trình du học công lập theo độ tuổi"}
        </h2>
      ...
      <div className="mt-12 bg-white border border-border/60 rounded-lg p-6 md:p-8 shadow-sm dark:border-border/10 dark:bg-card">
        ...
        <h3 className="font-heading text-xl font-bold text-brand-blue mt-2 mb-6 dark:text-foreground">
          {content?.compareTitle || "So sánh lộ trình Junior College (JC) vs Polytechnic (Poly)"}
        </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="flex flex-col gap-4 font-body text-sm leading-normal">
              <div>
                ...
                <span className="ml-1 text-muted-foreground">
                  {content?.compareCard1?.duration || "Chương trình dự bị đại học kéo dài 2 năm."}
                </span>
              </div>
              <div>
                <span className="font-semibold text-brand-blue">Định hướng đầu ra:</span>
                <span className="ml-1 text-muted-foreground">
                  {content?.compareCard1?.objective || "Chuẩn bị trực tiếp cho kỳ thi GCE A-Level để xét tuyển vào các đại học công lập hàng đầu."}
                </span>
              </div>
              <div>
                <span className="font-semibold text-brand-blue">Điều kiện xét tuyển:</span>
                <span className="ml-1 text-muted-foreground">
                  {content?.compareCard1?.criteria || "Xác nhận hồ sơ tài năng, học bạ cấp 2 và lớp 10/11, hoạt động ngoại khóa, bài luận cá nhân và tham gia kỳ thi J-PACT."}
                </span>
              </div>
              <div>
                <span className="font-semibold text-brand-blue">Học phí tham khảo:</span>
                <span className="ml-1 text-brand-gold font-semibold">
                  {content?.compareCard1?.feeReference || "Khoảng 1.200 – 1.400 SGD/tháng."}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="border border-border/40 rounded-lg p-5 bg-brand-light/20">
            <h4 className="font-heading text-lg font-bold text-brand-blue border-b border-border pb-3 mb-4 dark:text-foreground">
              {content?.compareCard2?.title || "Polytechnic (Cao đẳng công lập)"}
            </h4>
            <div className="flex flex-col gap-4 font-body text-sm leading-normal">
              <div>
                <span className="font-semibold text-brand-blue">Thời gian học:</span>
                <span className="ml-1 text-muted-foreground">
                  {content?.compareCard2?.duration || "Đào tạo chuyên môn/thực hành kéo dài 3 năm (nhận bằng Diploma)."}
                </span>
              </div>
              <div>
                <span className="font-semibold text-brand-blue">Định hướng đầu ra:</span>
                <span className="ml-1 text-muted-foreground">
                  {content?.compareCard2?.objective || "Làm việc trực tiếp sau khi tốt nghiệp, hoặc học liên thông lên Đại học (được giảm tín chỉ)."}
                </span>
              </div>
              <div>
                <span className="font-semibold text-brand-blue">Điều kiện xét tuyển:</span>
                <span className="ml-1 text-muted-foreground">
                  {content?.compareCard2?.criteria || "Xét điểm học bạ THPT trung bình từ 7.0 trở lên, đi kèm chứng chỉ IELTS tối thiểu 5.5 - 6.0."}
                </span>
              </div>
              <div>
                <span className="font-semibold text-brand-blue">Học phí tham khảo:</span>
                <span className="ml-1 text-brand-gold font-semibold">
                  {content?.compareCard2?.feeReference || "Khoảng 11.000 – 14.000 SGD/năm."}
                </span>
              </div>
            </div>
          </div>
