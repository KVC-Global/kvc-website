...
import type { PublicStudyIntroContent } from "@/sanity/public-study-page"

const DEFAULT_IMAGE = "https://images.unsplash.com/photo-1516321318423-f06f85e504b3?q=80&w=800&auto=format&fit=crop"

export function PublicStudyAbroadIntro({ content }: { content?: PublicStudyIntroContent }) {
  const imageUrl = content?.image
    ? urlFor(content.image).url()
    : DEFAULT_IMAGE

  const imageAlt = content?.imageAlt || "Học sinh học tập tại hệ thống trường công lập Singapore"
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col lg:col-span-7">
        ...
        >
          {content?.title || "Hệ thống giáo dục công lập Singapore là gì?"}
        </h2>
        ...
        <div className="mt-6 font-body text-sm leading-relaxed text-brand-dark/90 sm:text-base">
          ...
          <p className="mb-4">
            {content?.paragraph3 || "KVC Global đồng hành cùng phụ huynh và học sinh trong suốt hành trình: từ tư vấn lộ trình phù hợp, ôn thi luyện thi đầu vào (AEIS), hoàn thiện hồ sơ và xin Student Pass, đến hỗ trợ giám hộ và lưu trú lâu dài tại Singapore."}
          </p>
        </div>
      </div>
    </section>
